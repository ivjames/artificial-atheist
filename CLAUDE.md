...
- **THE ICON FONT IS A GENERATED SUBSET.** `app/icons.css` +
  `public/fonts/tabler-icons-subset.woff2` (~5KB, committed) are generated by
  `scripts/icon-subset.mjs` (`npm run icons:subset`), which scans app/,
  components/, and lib/ for the `ti-*` names actually used. NEVER import the
  full `@tabler/icons-webfont` CSS in layout.tsx — that's ~450KB of
  render-blocking font+CSS for ~30 icons, and was the Aug-2026 QA scan's
  biggest performance cost (post pages 0.73→0.96 in local Lighthouse after the
  subset). After using a NEW ti-* icon anywhere, re-run `npm run icons:subset`
  and commit both outputs — an un-subsetted glyph renders as a blank box.
- **CARD LINKS wrap the title only** (`.card-link` + a `::after` stretched-link
